...
import { ChevronRight, Search, Play, RotateCcw, Eye, Trash2 } from "lucide-react";

const tasks = [
  { id: "TK-001", name: "联邦训练-金融风控", type: "联邦学习", status: "运行中", progress: 65, startTime: "2026-04-24 09:00", node: "Node-A1" },
  { id: "TK-002", name: "PSI求交-客户比对", type: "PSI", status: "排队中", progress: 0, startTime: "-", node: "Node-B2" },
  { id: "TK-003", name: "联合统计-销售汇总", type: "联合统计", status: "已完成", progress: 100, startTime: "2026-04-23 14:00", node: "Node-A2" },
  { id: "TK-004", name: "评分卡推理-信贷评估", type: "评分卡", status: "失败", progress: 30, startTime: "2026-04-24 08:30", node: "Node-C1" },
];
    ...
    <div className="p-6 space-y-6 max-w-[1440px] mx-auto">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbItem><BreadcrumbLink>任务中心</BreadcrumbLink></BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>

      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900 tracking-tight">任务中心</h1>
      ...
      <Card>
        <CardContent className="pt-6">
          <Table className="unified-table">
            <TableHeader>
              <TableRow>
                <TableHead>任务ID</TableHead><TableHead>任务名称</TableHead><TableHead>类型</TableHead><TableHead>状态</TableHead>
                <TableHead>进度</TableHead><TableHead>开始时间</TableHead><TableHead>节点</TableHead><TableHead>操作</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filtered.map(t => (
                <TableRow key={t.id}>
                  ...
                  <TableCell>
                    <div className="w-24 bg-gray-200 rounded-full h-2"><div className="bg-blue-600 h-2 rounded-full" style={{ width: `${t.progress}%` }} /></div>
                    <span className="text-xs text-gray-500">{t.progress}%</span>
                  </TableCell>
                  <TableCell className="text-xs">{t.startTime}</TableCell>
                  <TableCell className="text-xs font-mono">{t.node}</TableCell>
                  <TableCell>
